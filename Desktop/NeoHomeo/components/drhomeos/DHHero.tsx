  ...
  return (
    <section className="relative flex flex-col items-center justify-center pt-16 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 w-full py-16 md:py-20 flex flex-col items-center">
        ...
        {/* Headline */}
        <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-center leading-[1.05] tracking-tight mb-5 px-2"
        ...
        <div className="h-7 flex items-center justify-center mb-5">
          <p className="text-base md:text-xl font-semibold text-center transition-opacity duration-300"
        ...
        {/* Subtext */}
        <p className="text-sm md:text-lg text-center max-w-xl mb-8 leading-relaxed px-2"
          style={{ color: "var(--text-dim)" }}>
          DrHomeos is a professional-grade AI study platform for BHMS students and practitioners.
          Master Materia Medica, Repertory, and Organon — the way the masters intended.
        </p>

        {/* CTAs */}
        <div className="flex flex-col sm:flex-row gap-3 mb-12 w-full sm:w-auto px-4 sm:px-0">
          <Link href="/register"
        ...
        {/* Stats row — 4-col on mobile too with shorter labels */}
        <div className="grid grid-cols-4 gap-2 md:gap-4 w-full max-w-3xl mb-12 px-2 md:px-0">
          {STATS.map((s) => (
            <div key={s.label} className="text-center rounded-2xl py-3 px-1 md:py-4 md:px-3"
              style={{ background: "rgba(255,255,255,0.5)", border: "1px solid var(--glass-border)", backdropFilter: "blur(12px)" }}>
              <div className="text-lg sm:text-2xl md:text-3xl font-black mb-0.5"
                style={{ color: "var(--text-obsidian)", fontFamily: "var(--font-mono)" }}>
                {s.value}
              </div>
              <div className="text-[9px] sm:text-xs font-semibold leading-tight" style={{ color: "var(--text-dim)" }}>{s.label}</div>
            </div>
          ))}
        </div>

        {/* Dashboard mockup — desktop full view, mobile card stack */}
        <div className="w-full max-w-5xl relative">

          {/* ── DESKTOP mockup (hidden on mobile) ── */}
          <div className="hidden md:block rounded-3xl overflow-hidden shadow-2xl border"
            style={{ background: "rgba(255,255,255,0.7)", borderColor: "var(--glass-border)", backdropFilter: "blur(20px)" }}>
            {/* Browser chrome */}
            <div className="flex items-center gap-2 px-4 py-3 border-b"
              style={{ borderColor: "var(--glass-border)", background: "rgba(255,255,255,0.8)" }}>
            ...
            <div className="flex" style={{ minHeight: "300px" }}>
              {/* Sidebar */}
              <div className="w-44 border-r flex-shrink-0 p-3 space-y-1"
                ...
                {[["🧪","Materia Medica"],["📚","Repertory"],["📖","Organon Tutor"],["🃏","Flashcards"],["❓","Quiz"],["📝","Notes"]].map(([icon, label]) => (
                  <div key={label} className="px-3 py-2 rounded-xl text-xs font-medium flex items-center gap-2"
                    style={{ color: "var(--text-dim)" }}>
                ...
                <div className="flex items-center justify-between flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="h-7 px-3 rounded-full text-xs font-bold flex items-center gap-1"
                      style={{ background: "rgba(78,115,223,0.1)", color: "var(--accent-mineral)" }}>🔥 7 day streak</div>
                    <div className="h-7 px-3 rounded-full text-xs font-bold flex items-center gap-1"
                      style={{ background: "rgba(138,43,226,0.1)", color: "#8A2BE2" }}>⚡ 420 XP</div>
                  </div>
                </div>
          ...
          {/* ── MOBILE mockup — clean card stack ── */}
          <div className="md:hidden rounded-3xl overflow-hidden shadow-xl border"
            style={{ background: "rgba(255,255,255,0.8)", borderColor: "var(--glass-border)" }}>
            {/* URL bar */}
            <div className="flex items-center gap-2 px-3 py-2.5 border-b"
              style={{ borderColor: "var(--glass-border)", background: "rgba(255,255,255,0.9)" }}>
              <div className="flex gap-1">
                <div className="w-2.5 h-2.5 rounded-full bg-red-400/80" />
                <div className="w-2.5 h-2.5 rounded-full bg-yellow-400/80" />
                <div className="w-2.5 h-2.5 rounded-full bg-green-400/80" />
              </div>
              <div className="flex-1 mx-2 h-5 rounded-lg flex items-center px-2 text-[10px] font-mono"
                style={{ background: "rgba(0,0,0,0.05)", color: "var(--text-dim)" }}>
                drhomeos.com/student
              </div>
            </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex flex-col items-end gap-1">
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                    style={{ background: "rgba(78,115,223,0.1)", color: "var(--accent-mineral)" }}>🔥 7 day streak</span>
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-full"
                    style={{ background: "rgba(138,43,226,0.1)", color: "#8A2BE2" }}>⚡ 420 XP</span>
                </div>
              </div>
              {/* 3 stat cards */}
              <div className="grid grid-cols-3 gap-2">
                {[
                  { icon: "🧪", label: "Today's Remedy", value: "Sulphur", color: "#4e73df" },
                  { icon: "🃏", label: "Cards Due", value: "12", color: "#8A2BE2" },
                  { icon: "📊", label: "Mastery", value: "74%", color: "#16a34a" },
                ].map((c) => (
                  <div key={c.label} className="rounded-xl p-2.5 text-center"
                    ...
                    <div className="text-[9px] font-medium mb-0.5" style={{ color: "var(--text-dim)" }}>{c.label}</div>
                    <div className="text-sm font-black" style={{ color: c.color }}>{c.value}</div>
                  </div>
                ))}
              </div>
              {/* AI snippet */}
              <div className="rounded-xl p-3" style={{ background: "rgba(78,115,223,0.04)", border: "1px solid rgba(78,115,223,0.12)" }}>
                <div className="flex items-center gap-2 mb-1.5">
                  <div className="w-5 h-5 rounded-lg flex items-center justify-center text-white text-[9px] font-bold flex-shrink-0"
                    style={{ background: "linear-gradient(135deg,#4e73df,#8A2BE2)" }}>H</div>
                  <span className="text-xs font-bold" style={{ color: "var(--text-obsidian)" }}>Hahnemann AI</span>
                  <span className="text-[9px] px-1.5 py-0.5 rounded-full font-bold ml-auto" style={{ background: "rgba(22,163,74,0.1)", color: "#16a34a" }}>AI</span>
                </div>
                <p className="text-[10px] leading-relaxed" style={{ color: "var(--text-dim)" }}>
                  <span className="font-semibold" style={{ color: "var(--text-obsidian)" }}>§153 teaches us</span> to seek the most characteristic, striking, and peculiar symptoms of the case…
                </p>
              </div>
            </div>
          </div>
